  ...
  <Sheet.Root open={isSheetOpen} onOpenChange={(val) => (isSheetOpen = val)}>
    <Sheet.Content side="left" class="w-64 p-0">
      <aside class="flex h-full flex-col border-r bg-background">
        ...
        <div class="p-4 border-t bg-background text-center text-xl font-semibold tracking-tight">
    ...
    <div class="flex-1 overflow-auto p-6">
      <div class="mx-auto max-w-4xl rounded-2xl border bg-background p-6 shadow-md">
        ...
        <div class="flex items-center border-b mb-4 overflow-x-auto">
          {#each notes as note}
            <div
              class="relative flex items-center px-4 py-2 text-sm cursor-pointer transition-colors"
          ...
          {/each}
          <button class="ml-2 text-muted-foreground hover:text-foreground" on:click={addNote}>
            <Plus class="size-4" />
          </button>
        </div>
        ...
        <div class="mb-4 flex justify-between items-center border-b pb-2 text-muted-foreground">
        ...
        <!-- Контент -->
        {#if isPreviewMode}
          <div class="prose prose-invert max-w-none" use:html={renderedMarkdown}></div>
        {:else}
          <Textarea
